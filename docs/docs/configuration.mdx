...
Server-side capture of the meeting — audio (a browser tap) **and** video (an `ffmpeg` x11grab of the
bot's virtual display), muxed into one downloadable master. Turn recording on per spawn or with
`RECORDING_ENABLED` (above); these variables tune HOW it is captured. Recording currently always
captures both audio and video together (there is no audio-only toggle yet).
