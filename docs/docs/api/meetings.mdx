...
<ParamField body="record_video" type="boolean">When recording, whether to include the **video** track. Same resolution pattern against env `RECORD_VIDEO` (default `false`). Default records **audio only** (the server-side video x11grab is skipped; audio is still recorded); `true` also captures video. Only consulted when `recording_enabled` resolves true.</ParamField>
